...
if(retrieve_saved_blogposts){
    ...
    let default_blogposts = [
        ...
        {id: 2, title: "Siphon Brewing: Brewing with Flare", img: "/img/s2.webp", date: "May 3, 2024", content: `Among the array of brew methods, few are as visually captivating and scientifically fascinating as siphon brewing. This theatrical technique combines elements of immersion and vacuum brewing to produce a remarkably clean and flavorful cup of coffee. If you're seeking a new brewing adventure that delights the senses, look no further than the siphon brewer.
        The Siphon Setup:
...
function renderBlogPost(blogpost){
    ...
    blogElement.classList.add('blog', '2xl:w-[90%]');
    blogElement.setAttribute('data-id', blogpost.id);

    blogElement.innerHTML = `
        <div id="dailystories" class="cursor-pointer p-4 text-sm font-light border border-s-[#d9d9d9] rounded-bl-[40px] rounded-tr-[96px] hover:shadow-lg xl:text-xs xl:p-3">
          
            <div class="grid place-items-center max-w-[370px] h-96 overflow-hidden rounded-tr-[80px] rounded-bl-[80px] mb-4 xl:h-64 lg:h-48">
                <img src="${blogpost.img}" alt="Stories" class="w-full h-full object-cover">
            </div>
           
            <div class="gap-32 2xl:gap-28 grid grid-flow-col justify-center xl:gap-10 lg:gap-0">
            ...
            <div class="max-w-[363px] h-[90px] font-plusjakartaSans font-semibold text-2xl mx-auto mt-3 flex items-start 2xl:text-lg xl:text-base xl:h-20">
    ...
    blogsContainer.forEach(container => {
        container.prepend(blogElement.cloneNode(true));
    });
}
        ...
        for(let i = 0; i < 3; i++){
        renderBlogPost(blogposts[i], false);
        }
...
window.addEventListener("hashchange", function() {
    ...
    if (history !== "#dailystories"){
        ...
        document.getElementById('readBlog').classList.add("none");
       
     }    
     if(history !== "#readMoreBlogs"){
        sessionStorage.removeItem('readmore');
        document.getElementById('home').style.display = "block";
        document.getElementById('readMoreBlogs').style.display = "none";
...
function storiesReadMore(){
    document.getElementById('readMoreBlogs').style.display = "block";
    document.getElementById('home').style.display = "none";
    blogposts.forEach(blogpost => renderBlogPost(blogpost));
    window.scrollTo(0, 0); 
}
...
function renderBlogPostTable(blogpost){
    ...
    deleteButton.onclick = function() {
        ...
        blogposts =  blogposts.filter(blogposts => blogposts.id != delete_blogpost);
    ...
    const tableBody = document.getElementById('blog-posts-table').querySelector('tbody');
...
function openUpdatePostPopup(blogpost){
    ...
    const updatePostImage = document.getElementById('update-post-image');
...
function closeUpdatePostPopup(){
    const updatePostPopup = document.getElementById('update-post-popup');
    updatePostPopup.classList.add('hidden'); // Hide the popup
}
